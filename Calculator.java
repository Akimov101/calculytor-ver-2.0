...
class Calc {
    ...
    public static String parse(String expression) throws Exception {
        ...
        if (ArabicNumeral.isArabicNumeral(operands[0]) && ArabicNumeral.isArabicNumeral(operands[1])) {
            //конвертируем оба числа в арабские для вычесления действия
            num1 = ArabicNumeral.convertToarabic(operands[0]);
            num2 = ArabicNumeral.convertToarabic(operands[1]);
            isRoman = true;
        }
        //если оба числа арабские
        else if (!ArabicNumeral.isArabicNumeral(operands[0]) && !ArabicNumeral.isArabicNumeral(operands[1])) {
        ...
        if (isRoman) {
            ...
            if (arabic <= 0) {
                throw new Exception("Римское число должно быть больше нуля и меньше 10");
            }
            //конвертируем результат операции из арабского в римское
            result = ArabicNumeral.convertToRoman(arabic);
        } else {
...
class ArabicNumeral {
    static String[] arabicArray = new String[]{"0", "I", "II", "III", "IV", "V", "VI", "VII", "VIII", "IX", "X", "XI",
    ...
    public static boolean isArabicNumeral(String val) {
        for (int i = 0; i < arabicArray.length; i++) {
            if (val.equals(arabicArray[i])) {
                return true;
            }
        }
        return false;
